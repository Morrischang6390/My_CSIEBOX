...
int server_sync_meta( const csiebox_protocol_meta req, int conn_fd, const csiebox_server *server )
{
	...
	int fd = open(path, O_WRONLY | O_CREAT, REG_S_FLAG );
	if ( flock( fd, LOCK_EX | LOCK_NB) == -1 && errno == EWOULDBLOCK){
		fprintf(stderr, "The file has been locked\n");
		res.res.status = CSIEBOX_PROTOCOL_STATUS_FAIL;
		send_message(conn_fd, &res, sizeof(res));
		char err_msg[256] = "Server block\n";
		if(!send_message(conn_fd, err_msg, sizeof(err_msg)))
			fprintf(stderr, "sending error msg: fail\n");
		close(fd);
		return -1;
	}
	else fprintf(stderr, "LOCK SUCCESS\n");
...
void server_busy_res( int conn_fd)
{

// clean socket descriptor
	csiebox_protocol_meta trash;
	char trash_item[4096] = {};
	memset(&trash, 0, sizeof(trash));
	recv_message(conn_fd, &trash, sizeof(trash));
	recv_message(conn_fd, &trash_item, trash.message.body.pathlen);
	fprintf(stderr, "finish clear\n");

// send to client for server busy state
	...
	char err_mesg[256] = "Server busy\n";
	if(!send_message(conn_fd, &err_mesg, sizeof(err_mesg)))
		fprintf(stderr, "sending error msg(busy): fail\n");
