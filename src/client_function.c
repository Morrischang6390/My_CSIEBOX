...
int client_sync_file(const char *path, const csiebox_client *client)
{
	...
	else
	{
		//return -1;
		char err_mesg[256];
		recv_message(client->conn_fd, &err_mesg, sizeof(err_mesg));
		fprintf(stdout, "%s", err_mesg);
		fflush(stdout);
		fprintf(stderr, "Start to sleep (3 seconds)\n");
		sleep(3);
...
int client_send_file( const char *path, const csiebox_client *client)
{
	...
	if ( flock(file_fd, LOCK_SH) == -1) fprintf(stderr, "ERROR: add shared lock\n");
	...
	if ( flock(file_fd, LOCK_UN) == -1) fprintf(stderr, "ERROR: Unlock shared lock\n");
	close(file_fd);
